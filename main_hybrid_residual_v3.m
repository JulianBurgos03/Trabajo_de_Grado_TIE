...
%% 0) Archivo y carga
phantom_file = 'C:\Users\A15\Downloads\ds_data_2004\objects\plastic_2_45.txt';
...
for k=1:K
  ...
  Xva_k = X(~isVal==0,:);  Tva_k = T(~isVal,:);   %#ok<NASGU> % para coherencia
...
% Configurar patrón de estimulación Adjacent (adyacente)
stim = mk_stim_patterns(n_elec, 1, '{ad}', '{ad}', {}, 1);

% CRÍTICO: Ajustar meas_select para el patrón "Partial Model"
for i = 1:length(stim)
    all_meas = 1:(n_elec);
    inj_elecs = find(stim(i).stim_pattern ~= 0);
    exclude = [inj_elecs; mod(max(inj_elecs), n_elec) + 1];
    valid_meas = setdiff(all_meas, exclude);
    
    if length(valid_meas) > n_meas_per_stim
        valid_meas = valid_meas(1:n_meas_per_stim);
    end
    
    meas_sel = zeros(n_meas_per_stim, n_elec);
    for j = 1:length(valid_meas)
        if j <= n_meas_per_stim
            meas_sel(j, valid_meas(j)) = 1;
            if valid_meas(j) < n_elec
                meas_sel(j, valid_meas(j)+1) = -1;
            else
                meas_sel(j, 1) = -1;
            end
        end
...
n_meas_expected = n_stim * n_meas_per_stim;
fprintf('Patrones de inyección: %d\n', n_stim);
fprintf('Medidas por patrón: %d\n', n_meas_per_stim);
fprintf('Total medidas esperadas: %d\n', n_meas_expected);
fprintf('Medidas en dataset: %d\n', numel(Y(:)));

%% 9) Preparar datos para reconstrucción
n_data = min(numel(Y(:)), n_meas_expected);

% Datos reales 16 electrodos
v_real_16 = Y(:);
if numel(v_real_16) ~= n_meas_expected
    fprintf('Ajustando datos de %d a %d medidas\n', numel(v_real_16), n_meas_expected);
    if numel(v_real_16) > n_meas_expected
...
% Voltaje de referencia (homogéneo)
v_homog = mean(abs(v_real_16));
v_ref = v_homog * ones(n_meas_expected, 1);

% Caso 2: 8FEM + 8EV baseline
Y_conjugada_base = Y; 
Y_conjugada_base(:,V) = YhatV_base;
v_conj_base = Y_conjugada_base(:);
...
% Caso 3: 8FEM + 8EV híbrido
Y_conjugada_hyb = Y;
Y_conjugada_hyb(:,V) = YhatV_hyb;
v_conj_hyb = Y_conjugada_hyb(:);
...
%% 10) Algoritmos de reconstrucción: 6 métodos

fprintf('\n== Iniciando reconstrucciones (6 métodos) ==\n');

% ========== 10.1: Back-Projection (simple, robusto) ==========
fprintf('Ejecutando Back-Projection...\n');
imdl_bp = imdl;
imdl_bp.solve = @inv_solve_backproj;
imdl_bp.inv_solve_backproj.type = 'naive';

try
    img_bp_real = inv_solve(imdl_bp, v_ref, v_real_16);
    img_bp_base = inv_solve(imdl_bp, v_ref, v_conj_base);
    img_bp_hyb  = inv_solve(imdl_bp, v_ref, v_conj_hyb);
    bp_ok = true;
    fprintf('  ✓ Back-Projection completado\n');
catch ME
    fprintf('  ✗ Error en Back-Projection: %s\n', ME.message);
    bp_ok = false;
end

% ========== 10.2: Gauss-Newton ==========
fprintf('Ejecutando Gauss-Newton...\n');
imdl_gn = imdl;
imdl_gn.solve = @inv_solve_diff_GN_one_step;
imdl_gn.hyperparameter.value = 0.03;

try
    img_gn_real = inv_solve(imdl_gn, v_ref, v_real_16);
    img_gn_base = inv_solve(imdl_gn, v_ref, v_conj_base);
    img_gn_hyb  = inv_solve(imdl_gn, v_ref, v_conj_hyb);
    gn_ok = true;
    fprintf('  ✓ Gauss-Newton completado\n');
catch ME
    fprintf('  ✗ Error en Gauss-Newton: %s\n', ME.message);
    gn_ok = false;
end

% ========== 10.3: NOSER ==========
fprintf('Ejecutando NOSER...\n');
imdl_noser = imdl;
imdl_noser.solve = @inv_solve_diff_GN_one_step;
imdl_noser.hyperparameter.value = 0.05;
imdl_noser.RtR_prior = @prior_noser;

try
    img_noser_real = inv_solve(imdl_noser, v_ref, v_real_16);
    img_noser_base = inv_solve(imdl_noser, v_ref, v_conj_base);
    img_noser_hyb  = inv_solve(imdl_noser, v_ref, v_conj_hyb);
    noser_ok = true;
    fprintf('  ✓ NOSER completado\n');
catch ME
    fprintf('  ✗ Error en NOSER: %s\n', ME.message);
    noser_ok = false;
end
...
try
    ...
    laplace_ok = true;
    fprintf('  ✓ Laplace completado\n');
catch ME
    fprintf('  ✗ Error en Laplace: %s\n', ME.message);
    laplace_ok = false;
end
...
try
    ...
    tik_ok = true;
    fprintf('  ✓ Tikhonov completado\n');
catch ME
    fprintf('  ✗ Error en Tikhonov: %s\n', ME.message);
    tik_ok = false;
end

% ========== 10.6: Total Variation (versión alternativa) ==========
fprintf('Ejecutando Total Variation...\n');
imdl_tv = imdl;

% Método alternativo si inv_solve_TV_pdipm falla
try
    % Intentar versión estándar primero
    imdl_tv.solve = @inv_solve_TV_pdipm;
    imdl_tv.inv_solve_TV_pdipm.alpha = 5e-3;
    imdl_tv.inv_solve_TV_pdipm.beta  = 1e-4;
    ...
    tv_ok = true;
    fprintf('  ✓ Total Variation completado\n');
catch
    % Si falla, usar implementación manual simplificada
    try
        fprintf('  Usando implementación alternativa de TV...\n');
        img_tv_real = solve_TV_alternative(imdl, v_ref, v_real_16, 0.01);
        img_tv_base = solve_TV_alternative(imdl, v_ref, v_conj_base, 0.01);
        img_tv_hyb  = solve_TV_alternative(imdl, v_ref, v_conj_hyb, 0.01);
        tv_ok = true;
        fprintf('  ✓ Total Variation (alternativo) completado\n');
    catch ME
        fprintf('  ✗ Error en Total Variation: %s\n', ME.message);
        tv_ok = false;
    end
end

%% 11) Cálculo de métricas ER, MAE y CC

fprintf('\n== Métricas de Reconstrucción (ER, MAE, CC) ==\n');
fprintf('Referencia: Reconstrucción con 16 electrodos reales\n\n');

% Inicializar arrays para tabla
metodos = {};
ER_base_vals = []; MAE_base_vals = []; CC_base_vals = [];
ER_hyb_vals = [];  MAE_hyb_vals = [];  CC_hyb_vals = [];

if bp_ok
    [er_bb, mae_bb, cc_bb] = metrics_reconstruction(img_bp_base.elem_data, img_bp_real.elem_data);
    [er_bh, mae_bh, cc_bh] = metrics_reconstruction(img_bp_hyb.elem_data, img_bp_real.elem_data);
    
    metodos = [metodos; 'Back-Projection'];
    ER_base_vals = [ER_base_vals; er_bb];   MAE_base_vals = [MAE_base_vals; mae_bb];   CC_base_vals = [CC_base_vals; cc_bb];
    ER_hyb_vals  = [ER_hyb_vals; er_bh];    MAE_hyb_vals  = [MAE_hyb_vals; mae_bh];    CC_hyb_vals  = [CC_hyb_vals; cc_bh];
...
if gn_ok
    [er_gb, mae_gb, cc_gb] = metrics_reconstruction(img_gn_base.elem_data, img_gn_real.elem_data);
    [er_gh, mae_gh, cc_gh] = metrics_reconstruction(img_gn_hyb.elem_data, img_gn_real.elem_data);
    
    metodos = [metodos; 'Gauss-Newton'];
    ER_base_vals = [ER_base_vals; er_gb];   MAE_base_vals = [MAE_base_vals; mae_gb];   CC_base_vals = [CC_base_vals; cc_gb];
    ER_hyb_vals  = [ER_hyb_vals; er_gh];    MAE_hyb_vals  = [MAE_hyb_vals; mae_gh];    CC_hyb_vals  = [CC_hyb_vals; cc_gh];
...
if noser_ok
    [er_nb, mae_nb, cc_nb] = metrics_reconstruction(img_noser_base.elem_data, img_noser_real.elem_data);
    [er_nh, mae_nh, cc_nh] = metrics_reconstruction(img_noser_hyb.elem_data, img_noser_real.elem_data);
    
    metodos = [metodos; 'NOSER'];
    ER_base_vals = [ER_base_vals; er_nb];   MAE_base_vals = [MAE_base_vals; mae_nb];   CC_base_vals = [CC_base_vals; cc_nb];
    ER_hyb_vals  = [ER_hyb_vals; er_nh];    MAE_hyb_vals  = [MAE_hyb_vals; mae_nh];    CC_hyb_vals  = [CC_hyb_vals; cc_nh];
...
%% 12) Visualización UNIFICADA - TODOS los métodos en una sola figura
fig_all = figure('Name','Reconstrucción EIT: Comparativa Completa','Color','w',...
                 'Position',[50 50 1800 1000]);

% Contador de subplot
sp_idx = 1;
n_metodos = sum([bp_ok, gn_ok, noser_ok, laplace_ok, tik_ok, tv_ok]);

for metodo_idx = 1:6
    switch metodo_idx
        case 1  % Back-Projection
            if bp_ok
                subplot(n_metodos, 3, sp_idx); show_fem(img_bp_real); 
                title('Back-Proj: 16 Real'); axis equal tight; colorbar;
                subplot(n_metodos, 3, sp_idx+1); show_fem(img_bp_base); 
                title('Back-Proj: 8+8 Base'); axis equal tight; colorbar;
                subplot(n_metodos, 3, sp_idx+2); show_fem(img_bp_hyb); 
                title('Back-Proj: 8+8 Híbrido'); axis equal tight; colorbar;
                sp_idx = sp_idx + 3;
            end
            
        case 2  % Gauss-Newton
            if gn_ok
                subplot(n_metodos, 3, sp_idx); show_fem(img_gn_real); 
                title('GN: 16 Real'); axis equal tight; colorbar;
                subplot(n_metodos, 3, sp_idx+1); show_fem(img_gn_base); 
                title('GN: 8+8 Base'); axis equal tight; colorbar;
                subplot(n_metodos, 3, sp_idx+2); show_fem(img_gn_hyb); 
                title('GN: 8+8 Híbrido'); axis equal tight; colorbar;
                sp_idx = sp_idx + 3;
            end
            
        case 3  % NOSER
            if noser_ok
                subplot(n_metodos, 3, sp_idx); show_fem(img_noser_real); 
                title('NOSER: 16 Real'); axis equal tight; colorbar;
                subplot(n_metodos, 3, sp_idx+1); show_fem(img_noser_base); 
                title('NOSER: 8+8 Base'); axis equal tight; colorbar;
                subplot(n_metodos, 3, sp_idx+2); show_fem(img_noser_hyb); 
                title('NOSER: 8+8 Híbrido'); axis equal tight; colorbar;
                sp_idx = sp_idx + 3;
            end
            
        case 4  % Laplace
            if laplace_ok
                subplot(n_metodos, 3, sp_idx); show_fem(img_laplace_real); 
                title('Laplace: 16 Real'); axis equal tight; colorbar;
                subplot(n_metodos, 3, sp_idx+1); show_fem(img_laplace_base); 
                title('Laplace: 8+8 Base'); axis equal tight; colorbar;
                subplot(n_metodos, 3, sp_idx+2); show_fem(img_laplace_hyb); 
                title('Laplace: 8+8 Híbrido'); axis equal tight; colorbar;
                sp_idx = sp_idx + 3;
            end
            
        case 5  % Tikhonov
            if tik_ok
                subplot(n_metodos, 3, sp_idx); show_fem(img_tik_real); 
                title('Tikhonov: 16 Real'); axis equal tight; colorbar;
                subplot(n_metodos, 3, sp_idx+1); show_fem(img_tik_base); 
                title('Tikhonov: 8+8 Base'); axis equal tight; colorbar;
                subplot(n_metodos, 3, sp_idx+2); show_fem(img_tik_hyb); 
                title('Tikhonov: 8+8 Híbrido'); axis equal tight; colorbar;
                sp_idx = sp_idx + 3;
            end
            
        case 6  % Total Variation
            if tv_ok
                subplot(n_metodos, 3, sp_idx); show_fem(img_tv_real); 
                title('TV: 16 Real'); axis equal tight; colorbar;
                subplot(n_metodos, 3, sp_idx+1); show_fem(img_tv_base); 
                title('TV: 8+8 Base'); axis equal tight; colorbar;
                subplot(n_metodos, 3, sp_idx+2); show_fem(img_tv_hyb); 
                title('TV: 8+8 Híbrido'); axis equal tight; colorbar;
                sp_idx = sp_idx + 3;
            end
    end
end

sgtitle('Comparativa de Reconstrucciones EIT: 16 Real vs 8FEM+8EV (Base y Híbrido)', ...
        'FontSize', 14, 'FontWeight', 'bold');

%% 13) Gráficos comparativos de métricas
if ~isempty(metodos)
    figure('Name','Análisis de Métricas','Color','w','Position',[100 100 1400 500]);
    
    % ER
    subplot(1,3,1);
    x = 1:length(metodos);
    bar(x, [ER_base_vals, ER_hyb_vals]);
    set(gca, 'XTickLabel', metodos, 'XTick', x);
    xtickangle(45);
    ylabel('Error Relativo (ER)');
    title('Comparación de ER');
    legend('Base', 'Híbrido', 'Location', 'best');
    grid on;
    
    % MAE
    subplot(1,3,2);
    bar(x, [MAE_base_vals, MAE_hyb_vals]);
    set(gca, 'XTickLabel', metodos, 'XTick', x);
    xtickangle(45);
    ylabel('MAE');
    title('Comparación de MAE');
    legend('Base', 'Híbrido', 'Location', 'best');
    grid on;
    
    % CC
    subplot(1,3,3);
    bar(x, [CC_base_vals, CC_hyb_vals]);
    set(gca, 'XTickLabel', metodos, 'XTick', x);
    xtickangle(45);
    ylabel('Correlación (CC)');
    title('Comparación de CC');
    legend('Base', 'Híbrido', 'Location', 'best');
    grid on;
    ylim([0.95 1.0]);  % Zoom para ver diferencias
end

%% 14) Guardar resultados
vars_to_save = {'imdl', 'fmdl', 'v_ref', 'v_real_16', 'v_conj_base', 'v_conj_hyb'};
if bp_ok, vars_to_save = [vars_to_save, {'img_bp_real', 'img_bp_base', 'img_bp_hyb'}]; end
if gn_ok, vars_to_save = [vars_to_save, {'img_gn_real', 'img_gn_base', 'img_gn_hyb'}]; end
if noser_ok, vars_to_save = [vars_to_save, {'img_noser_real', 'img_noser_base', 'img_noser_hyb'}]; end
if laplace_ok, vars_to_save = [vars_to_save, {'img_laplace_real', 'img_laplace_base', 'img_laplace_hyb'}]; end
if tik_ok, vars_to_save = [vars_to_save, {'img_tik_real', 'img_tik_base', 'img_tik_hyb'}]; end
if tv_ok, vars_to_save = [vars_to_save, {'img_tv_real', 'img_tv_base', 'img_tv_hyb'}]; end
if exist('Tbl_recon', 'var'), vars_to_save = [vars_to_save, {'Tbl_recon'}]; end
...
function img_out = solve_TV_alternative(imdl, v_ref, v_meas, alpha_tv)
    % Implementación simplificada de Total Variation usando iteraciones GN con penalización TV
    % Esto es un aproximado cuando inv_solve_TV_pdipm no está disponible
    
    % Usar Tikhonov con peso TV aproximado
    imdl_tv_alt = imdl;
    imdl_tv_alt.solve = @inv_solve_diff_GN_one_step;
    imdl_tv_alt.hyperparameter.value = alpha_tv;
    imdl_tv_alt.RtR_prior = @prior_laplace;  % Laplace es aproximación a TV
    
    img_out = inv_solve(imdl_tv_alt, v_ref, v_meas);
end